...
def main(train_loader, valid_loader, valid_balanced_dataloader, seed):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % config.VALID_INTERVAL == 0:
            ...
            db_index2 = davies_bouldin_score(time_features.detach().squeeze(), labeli)
            ch_index2 = calinski_harabasz_score(time_features.detach().squeeze(), labeli)
            slh_index2 = silhouette_score(time_features.detach().squeeze(), labeli)
            ...
            if (epoch + 1) % config.SAVE_INTERVAL == 0 and db_index2 < best_dbi and slh_index2 > best_sc and ch_index2 > best_chi:
                ...
                best_sc = slh_index2
                best_chi = ch_index2
                torch.save(attn_model.state_dict(), f'models/monoselfPAB_model_epoch_{epoch + 1}.pth')
