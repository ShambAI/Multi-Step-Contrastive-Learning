...
def initialize_model(seed):
    set_seed(seed)
    model = TimeDRL(input_size=args['feature_dim'], output_size=args['out_features'])
    return model
...
def main(train_loader, valid_loader, valid_balanced_dataloader, seed):
    ...
    for epoch in tqdm(range(1, num_epochs+1)):
        ...
        if epoch % config.VALID_INTERVAL == 0:
            ...
            attn_model.eval()
            with torch.no_grad():
                time_features = attn_model.encoder(images)
            ...
            if epoch % config.SAVE_INTERVAL == 0:
                torch.save(attn_model.encoder.state_dict(), f'models/{ds_name + str(seed)}_timedlr_model_epoch_{epoch}.pth')
